...
if not data.empty:
    ...
    if act1.button("🚀 Finalize and Save to Database"):
        if not client_name: st.error("Missing Client Name.")
        elif foil_qty_entered > 0 and foil_code == 0: st.error("⚠️ Foil Qty entered but Foil Code is 0.")
        else:
            record = {
    ...
    # --- ENHANCED DATABASE SEARCH & LOAD SECTION ---
    if not st.session_state.database.empty:
        st.markdown("---")
